...
START_CITATION_SHEET_SYNC=true
...
$isPostgres = $env:DATABASE_URL -like "postgres*"
$psqlExe = $null
...
if ($isPostgres) {
  ...
  Write-Host "MySQL/MariaDB mode detected. Skipping PostgreSQL schema push and dump import." -ForegroundColor Yellow
  Write-Host "The app will read PD live data directly from the configured MySQL database." -ForegroundColor Green
}

if ($SetupOnly) {
  Write-Host "Setup complete. Now run RUN_DISCORD_WINDOWS.bat or RUN_WINDOWS.bat." -ForegroundColor Green
  exit 0
}
