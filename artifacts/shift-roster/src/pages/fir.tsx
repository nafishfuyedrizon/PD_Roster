...
function OfficerPicker({
  label, color, placeholder, value, onChange, officers,
}: {
  label: string; color: "green" | "teal"; placeholder: string;
  value: string | null; onChange: (v: string) => void; officers: OfficerItem[];
}) {
  const [search, setSearch] = useState("");
  const inputRef = useRef<HTMLInputElement>(null);
  const filtered = search.trim()
    ? officers.filter((o) => o.name?.toLowerCase().includes(search.toLowerCase()))
    : officers.slice(0, 8);
  const borderSel = color === "green" ? "border-green-600/40 bg-green-600/20 text-green-200" : "border-teal-600/40 bg-teal-600/20 text-teal-200";
  const labelColor = color === "green" ? "text-green-400" : "text-teal-400";
  return (
    <div className="space-y-1.5">
      ...
      />
      <div className="max-h-32 overflow-y-auto space-y-0.5 rounded-md border border-border/40 p-1 bg-background/30">
        {filtered.length === 0 && (
          <p className="text-[11px] text-muted-foreground text-center py-2">No officers found</p>
        )}
        {filtered.map((o) => (
          <button
            key={o.id}
            ...
            className={`w-full text-left px-2 py-1.5 rounded text-xs transition-colors border ${
              value === o.name ? borderSel : "border-transparent hover:bg-secondary/50 text-foreground"
            }`}
          >
            {o.name}
          </button>
      ...
      {value && (
        <div className={`text-[11px] font-medium ${labelColor}`}>
      ...
      <div className="flex items-start justify-between gap-3 px-4 py-3 flex-wrap">
        <div className="flex items-start gap-3 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1">
            ...
            {status === "accepted" && fir.officerName && (
